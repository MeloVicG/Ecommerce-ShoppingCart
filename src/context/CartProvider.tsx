...
export type ReducterAction = {
    type: string,
    payload?: CartItemType,
}

const reducer = (state: CartStateType, action: ReducterAction):
CartStateType => {
    switch(action.type){
...
const useCartContext = (initCartState: CartStateType) => {
    ...
    const cart = state.cart.sort((a,b) => {
        const itemA = Number(a.sku.slice(-4)) //grabs the sku and item number 0001
        const itemB = Number(b.sku.slice(-4)) //grabs the sku and item number 0001
        return itemA - itemB
    })
